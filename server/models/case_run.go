...
func NewCaseRun(c *Case) (cr *CaseRun, err error) {
	var timeStamp = time.Now().Format(time.RFC850)
	...
	for _, name := range acts {
		cr.ActionNames = append(cr.ActionNames, name)
	}
	...
	err = cr.Save()
	if err != nil {
		cr.NewStatus("saving_failed", fmt.Sprintf("Cannot save new: %v", err))
		return
	}
	...
	err = cr.Save()
	if err != nil {
		cr.NewStatus("saving_failed", fmt.Sprintf("Cannot save queued: %v", err))
		return
	}
	return
}
